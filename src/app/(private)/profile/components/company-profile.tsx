...
import { deleteCompany, removeAuthToken } from "@/lib/auth";
...
export function CompanyProfile() {
  ...
  const handleDeleteCompany = async () => {
    ...
    try {
      ...
      toast.success(response.message || "Empresa deletada com sucesso!");
      ...
      if (error instanceof AxiosError && error.response) {
        ...
        if (error.response.status === 409) {
          toast.error("Não é possível deletar empresa com vagas ativas.");
          return;
        }

        const message = errorData.message || "Erro ao deletar empresa";
        toast.error(message);
      } else {
